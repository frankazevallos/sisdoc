...
<body>
    <div id="myPrintArea">             	
		...
		<h3 class="text-center">TRÁMITE DEL DOCUMENTO N° {{str_pad($documento->iddocumento, 4, '0', STR_PAD_LEFT)}}</h2>
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{$operar->getusuario->adm_nombre}} {{$operar->getusuario->adm_apellido}}</td>
							...
							@if($operar->oper_usuid_d != 0)
								<td>{{$operar->getusudestino->adm_nombre}} {{$operar->getusudestino->adm_apellido}}</td>
							@else
								<td class="text-center"> - </td>
							@endif
