            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        ...
                        <li><a href="{{url('tramite/administracion/usuarios/create')}}">Nuevo Usuario</a></li>
                    </ul>
                        ...
                        <table id="example1" class="table table-bordered table-mihover">
                            <thead style="background-color: #69C57B;">
                                <tr>
                                    ...
                                    <th class="text-center c4">F. NACIMIENTO</th>
                            ...
                            <tbody>
                            	@foreach($users AS $user)
                              	<tr onclick="marcar(this)">
                               		<td>{{$user->adm_nombre}}</td>
                               		<td>{{$user->adm_apellido}}</td>
                               		<td>{{$user->adm_dni}}</td>
                               		<td class="text-center">{{ date('d/m/Y', strtotime($user->adm_cumpleanos))}}</td>
                               		<td>{{$user->adm_email}}</td>
                               		<td>{{$user->getdependencia->depe_nombre}}</td>
                               		<td>{{$user->adm_cargo}}</td>
                               		<td class="text-center">
                                        <div class="timeline-footer">
                                            {!! Form::open([ 'method' => 'DELETE', 'action' => ['UsuariosController@destroy', $user->idadmin]]) !!} {{csrf_field()}}
                                                <a class="btn btn-primary btn-xs" href="{{url('tramite/administracion/usuarios/'.$user->idadmin.'/edit')}}" title="Editar"><i class="fa fa-edit"></i></a> |
