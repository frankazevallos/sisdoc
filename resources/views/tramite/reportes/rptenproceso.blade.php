		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="box box-success">
				<div class="box-header with-border">
					<h3 class="box-title"><b>Reporte Generado: Documentos en Proceso</b></h3>
				</div> 
				<div id="pagina" style="background-color: white;">    
					...
					<h3 class="text-center">REPORTE | DOCUMENTOS EN PROCESO</h2>
							...
							<table class="table">
								...
								<tbody>
									...
									<tr class="text-center">
										<td>{{str_pad($enproceso->oper_iddocumento, 4, '0', STR_PAD_LEFT)}}</td>	
										<td>{{ date('d/m/Y', strtotime($enproceso->getdocumento->docu_fecharegistro)) }}</td>
										<td>{{$enproceso->getdocumento->gettipodocumento->tdoc_abreviado}} / {{str_pad($enproceso->getdocumento->docu_numero_doc, '3', '0', STR_PAD_LEFT)}} / {{ date('Y', strtotime($enproceso->getdocumento->docu_fecharegistro)) }} / {{$enproceso->getdocumento->docu_siglas_doc}}</td>
										<td>{{$enproceso->getdocumento->docu_folios}}</td>	
										<td>{{$enproceso->getdependencia->depe_nombre}}</td>	
										<td>{{$enproceso->getdocumento->docu_firma}}</td>	
										<td>{{$enproceso->getdocumento->docu_cargo}}</td>	
										<td>{{$enproceso->getdocumento->docu_asunto}}</td>	
									</tr>	
									@endforeach
								</tbody> 
							</table>
