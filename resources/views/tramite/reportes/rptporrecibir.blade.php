		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="box box-success">
				<div class="box-header with-border">
					<h3 class="box-title"><b>Reporte Generado: Documentos por Recibir</b></h3>
				</div> 
				<div id="pagina" style="background-color: white;">    
					...
					<h3 class="text-center"> REPORTE | DOCUMENTOS POR RECIBIR </h3>
							...
							<table class="table">
								<thead>
									<tr class="info2">
										...
										<th class="text-center">USUARIO DEST.</th>
										<th class="text-center">PROVEIDO</th>
									</tr>
								</thead>

								<tbody>
									@foreach($porrecibirs AS $porrecibir)
									<tr class="text-center">
										<td>{{str_pad($porrecibir->oper_iddocumento, 4, '0', STR_PAD_LEFT)}}</td>	
										<td>{{ date('d/m/Y', strtotime($porrecibir->oper_fecha)) }}</td>
										<td>{{$porrecibir->getdocumento->gettipodocumento->tdoc_abreviado}} / {{str_pad($porrecibir->getdocumento->docu_numero_doc, '3', '0', STR_PAD_LEFT)}} / {{ date('Y', strtotime($porrecibir->getdocumento->docu_fecharegistro)) }} / {{$porrecibir->getdocumento->docu_siglas_doc}}</td>
										<td>{{$porrecibir->getdependencia->depe_nombre}}</td>
										<td>{{$porrecibir->getusuario->adm_nombre}} {{$porrecibir->getusuario->adm_apellido}}</td>	
										<td>{{$porrecibir->getdepdestino->depe_nombre}}</td>
										<td>{{$porrecibir->getusudestino->adm_nombre}} {{$porrecibir->getusudestino->adm_apellido}}</td>	
										<td>{{$porrecibir->oper_detalledestino}}</td>
									</tr>	
									@endforeach
								</tbody> 
							</table>
